...
'''
    ...
    generate following 6 txtfiles (in terminal) contrains all trainning samples and stores in workinf directory:
     ...
     ls -r signatures/train/*_auth/*.png > splitted_train_auth.txt
     ls -r signatures/valid/*_auth/*.png > splitted_valid_auth.txt
     ls -r signatures/test/*_auth/*.png > splitted_test_auth.txt
     ls -r signatures/test/*_forg/*.png > splitted_test_forg.txt
     ls -r signatures/valid/*_forg/*.png > splitted_valid_forg.txt
     ls -r signatures/train/*_forg/*.png > splitted_train_forg.txt

    open auth and forg files
    iterate all images, figure out the name number and stored in a seperate
'''
train_auth_path = 'splitted_train_auth.txt'
train_forg_path = 'splitted_train_forg.txt'
valid_auth_path = 'splitted_valid_auth.txt'
valid_forg_path = 'splitted_valid_forg.txt'
test_auth_path = 'splitted_test_auth.txt'
test_forg_path = 'splitted_test_forg.txt'
...
for i in range(1, total_name + 1):
    ...
    if (i <= 6):  # valid
        forg_names.append('temp_lists/valid_forg_' + str(i) + '.txt')
        print(forg_names[i - 1])
        # open(forg_names[i - 1], 'a').close()
        auth_names.append('temp_lists/valid_auth_' + str(i) + '.txt')
        print(auth_names[i - 1])
        # open(valid_forg_names[i - 1], 'a').close()

    elif (i <= 15):  # test
        forg_names.append('temp_lists/test_forg_' + str(i) + '.txt')
        print(forg_names[i - 1])
        # open(test_forg_names[i], 'a').close()
        auth_names.append('temp_lists/test_auth_' + str(i) + '.txt')
        print(auth_names[i - 1])
        # open(test_forg_names[i - 6 - 1], 'a').close()
    else:
        forg_names.append('temp_lists/train_forg_' + str(i) + '.txt')
        print(forg_names[i - 1])
        # open(train_forg_names[i - 15 - 1], 'a').close()
        auth_names.append('temp_lists/train_auth_' + str(i) + '.txt')
        print(auth_names[i - 1])
        # open(train_forg_names[i - 15 - 1], 'a').close()
        print(i)
...
#         name = re.match(
#             r'signatures/.*name_(.*?)_auth.*.png', line)
...
#         name = re.match(
#             r'signatures/.*name_(.*?)_forg.*.png', line)
#         print("index", int(name.group(1))-1)
